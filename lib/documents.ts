...
,
  ...
  { 
    ...
    title: "标签页 Tabs", 
    path: "/react/tabs", 
    content: "Tabs 组件用于组织和展示不同类别的内容，允许用户在不同的视图之间进行切换。 ## 基础用法 Tabs 组件由四个部分组成： - Tabs: 容器组件 - TabList: 标签列表容器 - Tab: 单个标签项 - TabPanel: 标签对应的内容面板 ## 胶囊模式 通过设置 capsule 属性，可以启用胶囊样式的标签页，这种样式更加紧凑且具有视觉焦点。 ## 带额外操作 可以通过 ex..." 
  },
  { 
    id: "tag", 
    title: "标签 Tag", 
    path: "/react/tag", 
    content: "Tag 组件用于标记和分类内容，可以作为关键词或小型状态指示器使用。 ## 基础用法 最简单的用法是直接使用 Tag 组件包裹文本内容。 ## 变体 Tag 组件提供两种变体：primary 和 default。 ## 可关闭标签 通过设置 closable 属性，可以让标签变成可关闭的状态。当用户点击关闭按钮时会触发 onClose 回调。 ## 交互示例 以下是一个更复杂的交互示例，展示了如何..." 
  },
  { 
    id: "textarea", 
    title: "文本域 Textarea", 
    path: "/react/textarea", 
    content: "用于多行文本输入。 ## 何时使用 - 需要输入多行文本时 - 需要限制字数的文本输入时 - 需要可清除功能的多行文本输入时 ## 示例 ### 基础用法 最简单的文本域用法。 ### 不同行数 可以设置不同的行数。 ### 可清除文本域 带清除图标的文本域，点击图标清除内容。 ### 字数限制 可以设置最大字数限制，并在右下角显示计数。 ### 禁用状态 展示禁用状态的文本域。 ## API #..." 
  },
  { 
    id: "timeline", 
    title: "时间轴 Timeline", 
    path: "/react/timeline", 
    content: "Timeline 组件用于垂直展示时间流信息，常用于项目里程碑、订单跟踪、版本历史等场景。 ## 基础用法 Timeline 组件由两个部分组成： - Timeline: 容器组件 - TimelineItem: 时间轴上的每一个节点 ## 内容位置 通过设置 side 属性可以控制内容的显示位置，支持 right（默认）、left 和 both 三种模式。 ### 内容在右边（默认） ### 内..." 
  },
  { 
    id: "toast", 
    title: "轻提示 Toast", 
    path: "/react/toast", 
    content: "Toast 组件用于向用户提供简短的操作反馈信息，会在一段时间后自动消失。 ## 基础用法 Toast 组件提供了四种类型的提示：success、fail、danger 和 loading。可以通过调用相应的方法来显示不同类型的消息。 ## 成功提示 使用 Toast.success() 方法显示成功的反馈信息。 ## 失败提示 使用 Toast.fail() 方法显示失败的反馈信息。 ## 危险..." 
  },
  { 
    id: "tooltip", 
    title: "文字提示 Tooltip", 
    path: "/react/tooltip", 
    content: "Tooltip 组件用于在用户将鼠标悬停在元素上时显示简短的提示信息。 ## 基础用法 Tooltip 组件需要包裹一个触发元素，并通过 content 属性设置提示内容。 ## 不同内容 Tooltip 支持不同长度和类型的提示内容。 ## 在按钮上使用 Tooltip 常用于为按钮提供额外的说明信息。 ## 在图标上使用 Tooltip 也常用于为图标提供说明信息。 ## 富文本内容 Tool..." 
  }
];